...
func NewHandler(grpcAddr string, stringer func(req, resp interface{}) ([]byte, error), opts ...google_golang_org_grpc.DialOption) (net_http.Handler, error) {
	...
	GeoLookupClient := NewGeoLookupClient(conn)
	GeoLookupServer := NewHTMLGeoLookupServer(GeoLookupClient, stringer)
	mux.HandleFunc("/GeoLookup/Lookup", GeoLookupServer.Lookup)
	return mux, nil
}

type htmlGeoLookup struct {
	client   GeoLookupClient
	stringer func(req, resp interface{}) ([]byte, error)
}

func NewHTMLGeoLookupServer(client GeoLookupClient, stringer func(req, resp interface{}) ([]byte, error)) *htmlGeoLookup {
	return &htmlGeoLookup{client, stringer}
}

var FormGeoLookup_Lookup string = `<div class="container"><div class="jumbotron">
	<h3>GeoLookup: Lookup</h3>
...
var nodeFactory = {"LookupReq_RootKeyword": buildLookupReq_RootKeyword(emptyIfNull(null)),}
	function buildLookupReq_RootKeyword(json) {
if (json == undefined) {
		return "";
	}
	
var s = '<div class="node" type="LookupReq_RootKeyword" fieldname="RootKeyword" repeated="false">';
s += '<div class="field form-group"><label class="col-sm-2 control-label">lat: </label><div class="col-sm-10"><input class="form-control" name="lat" type="number" step="any" '+setValue(0, json["lat"])+'/></div></div>';
				
s += '<div class="field form-group"><label class="col-sm-2 control-label">lon: </label><div class="col-sm-10"><input class="form-control" name="lon" type="number" step="any" '+setValue(0, json["lon"])+'/></div></div>';
	...
	var root = $(nodeFactory["LookupReq_RootKeyword"]);
	...
	$("#form > .children").html(buildLookupReq_RootKeyword(json));
	activateLinks(root);
	$("a[id=submit]").click(function(ev) {
		ev.preventDefault();
		c = getChildren($("#form"));
		j = JSON.stringify(c["RootKeyword"]);
		window.location.assign("./Lookup?json="+j);
	});
}
...
func (this *htmlGeoLookup) Lookup(w net_http.ResponseWriter, req *net_http.Request) {
	w.Write([]byte(Header(`GeoLookup`, `Lookup`)))
	...
	w.Write([]byte(FormGeoLookup_Lookup))
	if someValue {
		reply, err := this.client.Lookup(golang_org_x_net_context.Background(), msg)
		if err != nil {
			if err != io.EOF {
				w.Write([]byte("<div class=\"alert alert-danger\" role=\"alert\">" + err.Error() + "</div>"))
				return
